...
-- | The Agent Memory client handle and the request machinery every method shares:
...
-- | A configured Agent Memory client.
data AgentMemory = AgentMemory
  { spOptions :: !AgentMemoryOptions
  , spManager :: !Manager
  , spObo     :: !(Maybe Text)
  }

-- | Build a client. Creates a TLS capable connection manager.
newAgentMemory :: MonadIO m => AgentMemoryOptions -> m AgentMemory
newAgentMemory opts = liftIO $ do
  manager <- newManager tlsManagerSettings
  pure (AgentMemory opts { soEndpoint = stripTrailing (soEndpoint opts) } manager Nothing)
  where
...
getJSON :: MonadIO m => AgentMemory -> Text -> [(Text, Maybe Text)] -> m Value
getJSON sp path qs = liftIO (run sp True "GET" path qs Nothing)

-- | Perform a POST with a JSON body and decode the JSON response.
postJSON :: MonadIO m => AgentMemory -> Text -> [(Text, Maybe Text)] -> Value -> m Value
...
postJSON_ :: MonadIO m => AgentMemory -> Text -> [(Text, Maybe Text)] -> m Value
...
deleteJSON :: MonadIO m => AgentMemory -> Text -> [(Text, Maybe Text)] -> m Value
deleteJSON sp path qs = liftIO (run sp False "DELETE" path qs Nothing)

-- | Perform a request that returns no content.
requestNoContent :: MonadIO m => AgentMemory -> BS.ByteString -> Text -> [(Text, Maybe Text)] -> m ()
...
requestJSON :: MonadIO m => AgentMemory -> BS.ByteString -> Text -> [(Text, Maybe Text)] -> Maybe Value -> m Value
requestJSON sp method path qs body = liftIO (run sp (method == "GET") method path qs body)

-- Internal -------------------------------------------------------------------

run :: AgentMemory -> Bool -> BS.ByteString -> Text -> [(Text, Maybe Text)] -> Maybe Value -> IO Value
run sp idempotent method path qs mbody = go 0
  where
    ...
    attemptOnce :: IO (Either AgentMemoryError Value)
    attemptOnce = do
      ...
      in if code >= 200 && code < 300
           then if BL.null body
                  then Right Null
                         ...
                         Right v -> Right v
                         Left e  -> Left (AgentMemoryError code ConnectionFailed "invalid JSON response" (Just (T.pack e)))
    ...
    connErr msg = AgentMemoryError 0 ConnectionFailed "connection failed" (Just msg)
...
rawRequest :: MonadIO m => AgentMemory -> BS.ByteString -> Text -> [(Text, Maybe Text)] -> Maybe Value -> m BL.ByteString
rawRequest sp method path qs mbody = liftIO $ do
  let opts = spOptions sp
      url  = T.unpack (soEndpoint opts <> path)
  ereq <- try (parseRequest url) :: IO (Either SomeException Request)
  case ereq of
    Left e -> throwIO (AgentMemoryError 0 ConnectionFailed "connection failed" (Just (T.pack (show e))))
    Right base -> do
      let req = setQueryString (queryBytes qs) base
            { method          = method
            , requestHeaders  = buildHeaders sp method
...
problemError :: Int -> BL.ByteString -> AgentMemoryError
problemError code body =
  ...
  in AgentMemoryError code kind title detail
  where
...
buildHeaders :: AgentMemory -> BS.ByteString -> [(HeaderName, BS.ByteString)]
